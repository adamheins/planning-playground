...
class Bidirectional_RRT(RRG):
    ...
    def extend(self, start, goal, n=100, min_edge_len=0.5, max_edge_len=1, niu=1,  stop_early=True):
        ...
        while ta.graph.n + tb.graph.n < n:
            ...
            if not self.has_path_to_goal():
                ...
                if dist < max_edge_len and not self.workspace.edge_is_in_collision(v.coord, v_tb.goal):
                    v.connect(v_tb)
    ...
    def get_path_to_goal(self):
        ...
        if not self.has_path_to_goal():
            return None
        path_ta = self.ta.get_path_to_goal()
